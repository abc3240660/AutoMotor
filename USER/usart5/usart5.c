...
u8 receive5_str[USART5_REC_NUM];     //接收缓存数组,最大USART_REC_LEN个字节 
u8 uart5_byte_count=0;
...
****************************************************************************/
int fputxxc(int ch, FILE *f)   //重定向，让printf输出到串口  
{
    uart5SendChar(ch);
...
void UART5_IRQHandler(void)  
{
	...
	if(USART_GetITStatus(UART5, USART_IT_RXNE) != RESET)  //接收中断 
		{
				rec_data =(u8)USART_ReceiveData(UART5);         //(USART1->DR) 读取接收到的数据
        if(rec_data=='S')		  	                         //如果是S，表示是命令信息的起始位
				{
					uart5_byte_count=0x01; 
				}

			else if(rec_data=='E')		                         //如果E，表示是命令信息传送的结束位
				{
//					if(strcmp("Light_led1",(char *)receive5_str)==0)        LED1=0;	//点亮LED1
//					else if(strcmp("Close_led1",(char *)receive5_str)==0)   LED1=1;	//关灭LED1
//					else if(strcmp("Open_beep",(char *)receive5_str)==0)    BEEP=1; 	//蜂鸣器响
//					else if(strcmp("Close_beep",(char *)receive5_str)==0)   BEEP=0; 	//蜂鸣器不响
					
					for(uart5_byte_count=0;uart5_byte_count<32;uart5_byte_count++)receive5_str[uart5_byte_count]=0x00;
					uart5_byte_count=0;    
				}				  
			else if((uart5_byte_count>0)&&(uart5_byte_count<=USART5_REC_NUM))
				{
				   receive5_str[uart5_byte_count-1]=rec_data;
				   uart5_byte_count++;
				}                		 
   } 
} 
