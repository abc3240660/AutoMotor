...
u8 receive6_str[USART6_REC_NUM];     //接收缓存数组,最大USART_REC_LEN个字节 
...
****************************************************************************/
int fputc6(int ch, FILE *f)   //重定向，让printf输出到串口  
{
    uart6SendChar(ch);
...
void USART6_IRQHandler(void)  
{
	...
	if(USART_GetITStatus(USART6, USART_IT_RXNE) != RESET)  //接收中断 
        ...
        if(rec_data=='S')		  	                         //如果是S，表示是命令信息的起始位
				...
				{
//					if(strcmp("Light_led1",(char *)receive6_str)==0)        LED1=0;	//点亮LED1
//					else if(strcmp("Close_led1",(char *)receive6_str)==0)   LED1=1;	//关灭LED1
//					else if(strcmp("Open_beep",(char *)receive6_str)==0)    BEEP=1; 	//蜂鸣器响
//					else if(strcmp("Close_beep",(char *)receive6_str)==0)   BEEP=0; 	//蜂鸣器不响
